...
if not (hasattr(args, "e_history")):
    setattr(args, "e_history", [])
e_args = "\n".join(
    [
        f"\t{_arg.ljust(25)} -- {str(type(getattr(args, _arg))).ljust(15)} -- {getattr(args, _arg)}"
...
deets = deets._replace(
    e_repr_seed=e_repr_seed, e_stt_datetime=e_stt_datetime, e_strftime=e_strftime, e_details=e_details
)
...
def multiOrientationDataLoader(
    ...
    if return_dls:
        dls = multiDHG1428.dataloaders(
            ds_directory,
            bs=bs,
            worker_init_fn=e_seed_worker,
            generator=e_repr_gen,
...
def Logger(*args, stdout=True):
    ...
    with open(f"{deets.e_tag}.yml", "a") as f:
        print(_e_prefix, *args, file=f)  # write to logfile
    if stdout:
        print(*args)  # write to std.out, maybe?
...
def i_Logger(i, e_accuracy):
    ...
    str_metric_values = ""
    for _mv in arr_metric_values[idx_e_accuracy][2:]:
        str_metric_values += f"{_mv:.4f} - "
...
class outsidersCustomCallback(TrackerCallback):
    ...
    def after_epoch(self):
        ...
        if accuracy > self.i_accuracy:
            ...
            if self.verbose:
                print(
                    f"@{self.i_tag}: Model {self.monitor} improved >> i/{self.epoch:02d}/{self.e_epochs:02d}/{self.i_accuracy:.4f}"
                )

        if args.create_e_tb_events:
            self.record_metric_values()
        self.e_epochs += 1
    ...
    try:
        ...
        if "CUDA" in str(e):
            ...
            if n_attempts == 69:
                ...
                Logger(f"i_LRFinder Exception:: {e}!")
                os._exit(os.EX_OK)
...
def FitFlatCosine(learn, i_tag, i_eps, i_pct_start, e_epochs_lr_accuracy, finetune=False, log=False, verbose=False):
    ...
    if finetune:
        i_lr, i_tag = (e_lr / 10), f"{i_tag}f"
    else:
        e_lr = i_lr = getLR(learn)
    ...
    if verbose:
        learn.show_results(nrows=1, ncols=4)
    if log:
        i_Logger(i_tag, oCCb.i_accuracy)
...
def initTraining(query_tuple, nd, bs, img_sz, eps=None, pct_start=None, log=False):
    ...
    if eps is None:
        _eps_a = _eps_b = opt_InitEpochs[nd]
    else:
        _eps_a, _eps_b = eps if type(eps) is list else [eps, eps]
    print(f"Batch Size: {bs}, Image Size: {img_sz}, Epochs: [{_eps_a}, {_eps_b}]")

    if pct_start is None:
        _ps_a, _ps_b = opt_pctStart[nd][_eps_a], opt_pctStart[nd][_eps_b]
    else:
        _ps_a, _ps_b = pct_start if type(pct_start) is list else [pct_start, pct_start]

    learn.freeze()
    _tag = f"i{img_sz}{isFrozen(learn)}"
...
def iterativeScaling(query_tuple, nd, bs, img_sz, eps, pct_start=None, finetune=False):
    ...
    if hasattr(learn.model, "tuner_img_sz"):
        learn.model._change_tuner_img_sz(size=img_sz)
    ...
    if pct_start is None:
        if eps in [3, 5, 10, 15, 20]:
            _ps = opt_pctStart[nd][eps]
        else:
            raise Exception("ValueError: valid `eps` options are [3, 5, 10, 15, 20]")
    else:
        _ps = pct_start

    learn.freeze()
    _tag = f"{img_sz}{isFrozen(learn)}"
    return_tuple = FitFlatCosine(
        learn,
        i_tag=_tag,
        i_eps=eps,
...
class end2endTunerModel(Module):
    ...
    def batch_tuner_images(self, preds):
        ...
        for _idx, _X in enumerate(preds):
            if self.debug and (_idx == 0):
                print(f"@batch_tuner_images: {_X.shape=} \n>> {_X=}")
...
def e2eTunerLearnerEvaluation(learn, mVOs, dls=None, ds_idx=1, verbose=False):
    ...
    clear_output(wait=False)
    if verbose:
        Logger(f"accuracies@e2eT: {accuracy}")
    else:
...
def _create_mvo_metric(idx: int, name: str):
    exec(
        dedent(
            f"""

        def accuracy{name}(self, preds, targs):
            return accuracy(preds[{idx}], targs)

        """
        )
    )
    return locals()[f"accuracy{name}"]


def attachMetrics(lossWrapper, mVOs, rename=False):
    for _idx, _vo in enumerate(mVOs):
        _vo = _vo.title().replace("-", "")
        setattr(lossWrapper, f"accuracy{_vo}", _create_mvo_metric(idx=_idx, name=_vo))
        if rename:
...
class e2eTunerLossWrapper(Module):
    ...
    def decodes(self, preds):
        return [_preds.argmax(dim=-1) for _preds in preds]
